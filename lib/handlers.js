...
// Note: uses an array + indexOf rather than an Object.create(null) lookup
// because Node 0.10's V8 treats __proto__ as a magic setter even on
// null-prototype objects, so the key is silently swallowed.
var UNSAFE_PROPERTY_NAMES = ['constructor', '__proto__', 'prototype'];

function isUnsafePropertyName(name) {
  if (typeof name !== 'string') return false;
  for (var i = 0; i < UNSAFE_PROPERTY_NAMES.length; i++) {
    if (UNSAFE_PROPERTY_NAMES[i] === name) return true;
  }
  return false;
}
